...
for line in lines:
    id = line["id"]
    if (
        ("positives" not in line)
        or ("negatives" not in line)
        or ("explanation" not in line)
    ):
        print(id, "not in decqa")
        not_found += 1
    elif (
        ("positives" in data[id])
...
split_files = [
    "./author_split/train_ids.txt",
    "./author_split/val_ids.txt",
    "./author_split/test_ids.txt",
]
